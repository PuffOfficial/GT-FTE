GTCEuStartupEvents.registry('gtceu:machine', event => {
	...
	event.create('primitive_electric_furnace', 'multiblock')
		.rotationState(RotationState.NON_Y_AXIS)
		.recipeType(GTRecipeTypes.FURNACE_RECIPES)
		.recipeModifiers([(machine, recipe) => ULVParallel(machine,recipe,8)])
		...
		.pattern(definition => FactoryBlockPattern.start()
			...
			.build())
		.workableCasingModel("kubejs:block/casings/solid_wrought_iron/solid_wrought_iron_casing", "gtceu:block/machines/electric_furnace")
		.additionalDisplay((controller, components) => {	
			if (controller instanceof $WorkableMultiblockMachine && controller.isFormed()) {
				components.add(Component.translatable("gtfte.components.parallels", Text.of(`§58`)));
			}
		})
});
